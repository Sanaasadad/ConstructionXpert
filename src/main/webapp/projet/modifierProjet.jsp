...
<div class="container mt-5">
  ...
  <form action="projet?action=update" method="post" class="border p-4 rounded shadow">
    <input type="hidden" name="id" value="<%= ((Projets) request.getAttribute("projet")).getId() %>">
    <div class="mb-3">
      <label class="form-label">Nom du projet</label>
      <input type="text" name="nom" class="form-control" value="<%= ((Projets) request.getAttribute("projet")).getNom() %>" required>
    </div>
    <div class="mb-3">
      <label class="form-label">Description</label>
      <textarea name="description" class="form-control" required><%= ((Projets) request.getAttribute("projet")).getDescription() %></textarea>
    </div>
    <div class="mb-3">
      <label class="form-label">Date Début</label>
      <input type="date" name="dateDebut" class="form-control" value="<%= ((Projets) request.getAttribute("projet")).getDateDebut() %>" required>
    </div>
    <div class="mb-3">
      <label class="form-label">Date Fin</label>
      <input type="date" name="dateFin" class="form-control" value="<%= ((Projets) request.getAttribute("projet")).getDateFin() %>" required>
    </div>
    <div class="mb-3">
      <label class="form-label">Budget (DH)</label>
      <input type="number" name="budget" class="form-control" value="<%= ((Projets) request.getAttribute("projet")).getBudget() %>" required>
    </div>
